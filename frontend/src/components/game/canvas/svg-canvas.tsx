    ...
    if (lastMessage) {
      try {
        ...
        switch (parsedMessage.type) {
          ...
          case "drawing": {
            setPencilDraft((prev) => {
              const newPath = parsedMessage.payload.path;

              // If there's no previous path, just use the new one
              if (!prev) {
                return {
                  stroke: parsedMessage.payload.stroke,
                  strokeWidth: parsedMessage.payload.strokeWidth,
              ...
              // Otherwise, append the new path to the previous one
              return {
                ...prev,
                path: `${prev.path} ${newPath}`,
              };
            });
            break;
          }
